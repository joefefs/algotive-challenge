...
export default function VideoCard({ video, onOpen }: VideoCardPropsType) {
  const { title, author, release_date, id } = video;

  return (
    <>
      <div className="grid grid-cols-4 gap-4 h-20 border-2 rounded-2xl m-10 items-center px-10 shadow-lg shadow-slate-100">
        <p>{title}</p>
        <p className="border-l-2">{author}</p>
        <p className="border-l-2">{fromatDate(release_date)}</p>
        <button
          onClick={() => onOpen(id)}
          className="flex justify-center border-l-2">
          <PlayIcon />
        </button>
      </div>
    </>
